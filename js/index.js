	...
	$.getJSON("data/flattenedSave.json", function (data) {
		...
		$('.dataTables_filter input').unbind().bind("input", function (e) {
			...
			if ($(this).val().length == 0)
				...
				datatable.search(removeDiacritics($(this).val())).draw();
		});
...
function getIconHTML(extension){
	if(extension == '.docx') return '<i class="fas fa-file-word fa-2x text-primary"></i><br>word';
	if(extension == '.xlsx') return '<i class="fas fa-file-excel fa-2x text-success"></i><br>excel';
	if(extension == '.pdf') return '<i class="fas fa-file-pdf fa-2x text-danger"></i><br>pdf';
